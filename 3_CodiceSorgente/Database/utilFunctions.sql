...
RETURNS TABLE (id bigserial, authorId text, nome text, durata integer, nomePuntoIniziale text,
...
CREATE OR REPLACE FUNCTION get_last_n_itinerario_starting_from(startingFrom integer, numberOfItinerarioToGet integer)
RETURNS TABLE (id bigserial, authorId text, nome text, durata integer, nomePuntoIniziale text,
difficolta difficoltaItinerario, descrizione text, tracciatoKey text)
LANGUAGE plpgsql
AS $$
	BEGIN
		RETURN QUERY EXECUTE ('SELECT * FROM Itinerario WHERE Itinerario.id <= $1 ORDER BY Itinerario.id LIMIT $2)' 
